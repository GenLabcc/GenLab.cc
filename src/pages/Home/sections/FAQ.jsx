...
export default function FAQ() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Blur and opacity reveal when entering
      gsap.fromTo(
        imageRef.current,
        { opacity: 0, filter: "blur(15px)" },
        {
          ...
          scrollTrigger: {
            trigger: containerRef.current,
            start: "top 80%", // Trigger when top of section is 80% down the screen
  ...
  return (
    <div>
      {/* meet */}
      <section id="capabilities" className={`${styles.white} ${styles.meet}`} ref={containerRef}>
        <div className={styles.left}>
          <img ref={imageRef} src={photo1} alt={"Henrich Profile"} />
        </div>
